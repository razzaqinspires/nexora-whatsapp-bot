...
export async function compactMemory(){await init();const events=await readMemory({limit:100000});const summary={version:"15.0.3",compactedAt:new Date().toISOString(),events:events.length,lastEvent:events.at(-1)?.ts||null,types:{}};for(const e of events)summary.types[e.type]=(summary.types[e.type]||0)+1;await fs.writeFile(snap,JSON.stringify(summary,null,2));return summary;}
