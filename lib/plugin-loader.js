...
export async function loadPlugins() {
  ...
  for(const file of files){
    try {
      ...
      game: new Set(['profile','character','inventory','gacha','equip','unequip','use','leaderboard','dashboard','map','enemy','dungeon','battle','skill','battleproof','flee','attack','duel','tournament','story','quest','achievements','guild','mario','runner','platformer']),
    ...
    category ||= (path.dirname(relative) === '.' ? 'core' : path.dirname(relative).split(path.sep)[0]);
      loaded.push(Object.freeze({...plugin, version: plugin.version || '17.0.0', category, __file:file}));
    } catch(error) {
