...
export default {name:'jadibot',version:'17.0.0',aliases:['jbot','subbot'],usage:'jadibot start <nomor> [hari]|status|stop',premiumOnly:true,category:'premium',help:'Premium: jadikan nomor sebagai bot dengan masa aktif/expired',async execute({m,args,config}){
 const action=String(args[0]||'status').toLowerCase(); const owner=m.identity?.pn||m.sender;
 if(action==='status'){const s=jadibotStatus(owner);if(!s)return m.reply({text:'Jadibot: OFF\nGunakan .jadibot start <nomor> [hari].'});return m.reply({text:`*JADIBOT v17.0*\nStatus: ${s.connected?'ONLINE':'OFFLINE'}\nBot: ${s.botJid||'-'}\nExpired: ${s.expiresAt||'-'}\nPairing: ${s.pairingCode||'-'}`});}
