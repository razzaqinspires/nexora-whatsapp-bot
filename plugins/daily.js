...
export default { name: 'daily', version: '17.0.0', aliases: ['claim'], usage: 'daily', category: 'economy', help: 'Daily reward', async execute({ m, prefix }) {
  try { const r = await dailyClaim(m); return sendButtons(m, { text: `*DAILY REWARD v17*\n+${r.xp} XP\n+${r.coins} coins\nTanggal: ${r.date}`, footer: 'NEXORA • Daily', buttons: [button(`${prefix}quest`, 'Quest'), button(`${prefix}profile`, 'Profile')] }); }
