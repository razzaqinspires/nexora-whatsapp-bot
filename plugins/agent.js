...
export default {name:'agent',version:'15.0.3',aliases:['agentai'],usage:'agent <request>',category:'ai',help:'NEXORA Agent dengan tool routing',async execute(ctx){const r=await agentRespond(ctx.rawArgs,{m:ctx.m,router:ctx.router,prefix:ctx.prefix});return ctx.m.reply({text:r.ok?`*NEXORA AGENT*\nProvider: ${r.provider}\n\n${r.text}`:`*NEXORA AGENT*\nStatus: ${r.code}\nProvider: ${r.provider||'-'}`});}};
